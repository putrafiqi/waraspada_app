...
class TrenTingkatStresKaryawanWidget extends StatelessWidget {
  const TrenTingkatStresKaryawanWidget({
    super.key,
      ...
      child: Column(
        ...
        children: [
          ...
          AspectRatio(
            aspectRatio: 16 / 9,
            child: BlocConsumer<
              TrenTingkatStresKaryawanBloc,
              TrenTingkatStresKaryawanState
            >(
              listener: (context, state) {
                if (state.status == TrenTingkatStresKaryawanStatus.failure) {
                  ShowToast.showEror(state.errorMessage ?? 'Terjadi kesalahan');
                }
              },
              builder: (context, state) {
                if (state.status == TrenTingkatStresKaryawanStatus.loading) {
                  return const Loader();
                }

                return _buildLineChart(context, state.data);
              },
            ),
          ),
        ],
